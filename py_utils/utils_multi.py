...
def post_processing(maps, threshold):
    ...
    for map_ in maps:
        ...
        if len(contours)>0:
            ...
            # 2nd Biggest Contour
            try:
                area2, snd_contour = max(contour_sizes, key=lambda x: x[0])
            except ValueError:
                area2 = 0
                apply_map1 = np.zeros(map_.shape, np.uint8)
                cX1, cY1 = -1, -1
            else:
                img = np.zeros(map_.shape, np.uint8)
                cv2.drawContours(img, [snd_contour], -1, (1), cv2.FILLED)
                apply_map1 = map_ * img
...
def rearange(array):
    ...
    for t in array:
        try:
            sorted_t = sorted(t, key=_helper)
        except TypeError:
            sorted_t = sorted(t)
        for t_sub in sorted_t:
            tmp.append(t_sub)
    return tmp
...
def tp_fp_tn_fn_alt(actual_centers, predicted_centers, maps_area, min_radius):
    ...
    TP, FP, TN, FN = 0, 0, 0, 0
    maps_area = rearange(maps_area)
    actual_centers = rearange(actual_centers)
    predicted_centers = rearange(predicted_centers)
    for area, ac, pc in zip(maps_area, actual_centers, predicted_centers):
        a_x, a_y = ac
        p_x, p_y = pc
